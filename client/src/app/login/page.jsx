...
export default function LoginPage() {
  ...
  const [error, setError] = useState(''); // Added error state to display errors
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:3003/api/users/login', { // Use the correct API URL
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        alert("Login Successful! Welcome back, " + data.data.name + "!"); // Greet user by name
